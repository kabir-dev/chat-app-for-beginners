...
export default function Home() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        const { data } = await instance.post("/api/login", obj);
  ...
  return (
    <div className=" h-screen dark:bg-slate-800">
      ...
      <div className="sm:shadow-md w-full sm:p-4 dark:shadow-none rounded-md mt-32 sm:w-96 m-auto">
        <div className="">{serverError && <Modal text={serverError} />}</div>
        <form onSubmit={handleSubmit}>
          ...
          <div className=" w-11/12 mx-auto">
            <input
              className=" w-full cursor-pointer px-4 py-3 outline-none rounded-lg text-white bg-sky-600"
              type="submit"
              value="Login"
            />
          </div>
